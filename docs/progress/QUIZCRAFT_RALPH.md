...
- `source_answer_id` 唯一约束避免同一作答重复生成错题卡；FSRS 调度和列表 UI 留后续增量。
- 验证：新闪卡 API 测试红灯为 404；实现后 `backend/tests/test_flashcards_api.py` 绿；相关答题测试 22 绿；
  后端全量 201 绿。
- `POST /api/flashcards/from-concepts` 可从 concept ids 幂等创建 `origin=concept`、`priority=normal`、
...
- 补齐填空/简答错题卡 public API 行为：同一未完成会话里先答错会生成 `origin=wrong_answer`、
  `priority=elevated` 且保留 source_span 的闪卡；随后改答正确会删除对应 source answer 的 stale
...
### TDD Issue #22（Flashcard management and review settings）

- 新增 review settings 后端 public API：`GET /api/settings/review` 返回默认值，`PUT /api/settings/review`
  持久化 `desired_retention`、`daily_new_limit`、`daily_review_limit` 到 settings KV 表。
- `GET /api/flashcards/due` 现在按 due cards 的 `state == "new"` 与 review/relearning/learning 分桶，
  分别应用 daily new limit 与 daily review limit，再返回合并后的到期列表。
