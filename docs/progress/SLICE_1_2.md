...
子系统 1「LLM 配置 UI 与后端」的**后端核心**完成：API key Fernet 加密存储 + KV settings 表 +
GET/POST /api/settings/llm（脱敏读 + 加密写 + 立即连通测试）。新增 29 测全绿（累计 100），
真机冒烟四条路径全通（含 SOCKS 代理环境 openai 构造失败的优雅降级修复）。

前端 Settings UI、运行时出题/答题改用 DB 配置、出题参数控制、简答评分、题目预览编辑、
交叉出题、完整 6 维自评——均待后续轮次。
...
切片 1.2 子系统 1 后端完成，剩余子系统 1 前端 + 子系统 2-6。下一轮优先做子系统 1 前端 Settings 页
（打通"配置→测试"端到端 UI），或子系统 2 出题参数控制（后端逻辑 + API，LLM mock 可测）。
